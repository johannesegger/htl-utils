    ...
    .flatMap(group => group.persons.map((person) : Person => (
      {
        ...
        imageUrl: person.imageUrl !== null ? { type: 'link', url: person.imageUrl } : undefined,
        isSelected: true,
      }
    )))
})
...
watch(allPersons, allPersons => {
  remainingPersons.value = shuffle(allPersons.filter(v => v.imageUrl != undefined))
  nextPerson.value = remainingPersons.value.pop()
})

watch(nextPerson, async nextPerson => {
  if (nextPerson?.imageUrl?.type !== 'link') return
  const response = await uiFetch(nextPerson.imageUrl.url)
  if (response.ok) {
    const image = await response.blob()
    nextPerson.imageUrl = { type: 'blob', url: URL.createObjectURL(image)}
  }
})
...
const selectablePersons = computed(() => {
  return filteredPersons.value.filter(v => v.imageUrl !== undefined)
})
...
<template>
  <head class="grid grid-cols-3 items-center p-8 bg-amber-200">
  ...
  <main class="grow flex flex-col gap-4 p-4 min-h-0">
    ...
    <div class="grow grid grid-cols-[2fr_1fr] gap-2 min-h-0">
      <template v-if="allPersons.length > 0">
        <div class="grow shadow-xl/30 p-4 min-h-0">
          <img v-if="nextPerson?.imageUrl?.type === 'blob'"
            :src="nextPerson.imageUrl.url"
        ...
        <div class="flex flex-col gap-2 px-2 py-4 shadow-xl/30 min-h-0">
          ...
          <div class="overflow-y-auto">
            <SelectablePerson v-for="person in filteredPersons" :key="person.id"
              ...
              :is-selectable="person.imageUrl !== undefined"
    ...
    <div class="flex justify-end gap-1">
      <div v-for="guess in guesses" class="rounded flex flex-col items-center p-2 gap-1 min-w-fit" :class="{ 'bg-green-100': guess.correct === true, 'bg-red-100': guess.correct === false }">
        <img v-if="guess.personToGuess.imageUrl?.type === 'blob'"
          :src="guess.personToGuess.imageUrl.url"
