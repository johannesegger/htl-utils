...
module DirectoryModification =
    let encode = function
        ...
        | UpdateUser (userName, userType, update) -> Encode.object [ "updateUser", Encode.tuple3 UserName.encode UserType.encode UserUpdate.encode (userName, userType, update) ]
        | DeleteUser (userName, userType) -> Encode.object [ "deleteUser", Encode.tuple2 UserName.encode UserType.encode (userName, userType) ]
    ...
    let decoder : Decoder<_> =
        Decode.oneOf [
            ...
            Decode.field "updateUser" (Decode.tuple3 UserName.decoder UserType.decoder UserUpdate.decoder) |> Decode.map UpdateUser
            Decode.field "deleteUser" (Decode.tuple2 UserName.decoder UserType.decoder) |> Decode.map DeleteUser
